...
// OHLC diario desde Binance (límites generosos, sin key); fallback a CoinGecko
const BINANCE_PAIR = {
...
export async function cryptoOhlc(id, days = 365) {
  return cached(`ohlc:${id}:${days}`, 10 * 60_000, async () => {
    ...
    if (pair) {
      try {
        const url = `https://api.binance.com/api/v3/klines?symbol=${pair}&interval=1d&limit=${days}`;
        const raw = await binanceQueued(() => getJsonRetry(url));
        return raw.map(k => ({
        ...
        console.warn(`[binance] ${pair} falló, fallback a CoinGecko: ${e.message}`);
      }
    }
...
const binanceQueued = makeQueue(150);
